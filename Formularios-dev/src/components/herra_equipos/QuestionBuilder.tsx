...
interface FormTemplate extends FormBuilderData {
  _id: string;
  createdAt: Date;
  updatedAt: Date;
}

const DEFAULT_SI_NO_NA_OPTIONS: ResponseOption[] = [
...
const QuestionBuilder: React.FC<QuestionBuilderProps> = ({
  ...
  const handleTypeChange = (newType: ResponseType) => {
    ...
    setValue(typeKey as keyof FormBuilderData, newType as never);
    if (newType === "si_no_na") {
      const optionsKey = `${sectionPath}.questions.${questionIndex}.responseConfig.options`;
      setValue(
        optionsKey as keyof FormBuilderData,
  ...
  const handleTextChange = (text: string) => {
    ...
    setValue(textKey as keyof FormBuilderData, text as never);
  };

  const handleObligatorioChange = (checked: boolean) => {
    const obligatorioKey = `${sectionPath}.questions.${questionIndex}.obligatorio`;
    setValue(obligatorioKey as keyof FormBuilderData, checked as never);
  };
  ...
  const handleSaveImage = () => {
    ...
    setValue(
      imageKey as keyof FormBuilderData,
  ...
  const handleRemoveImage = () => {
    const imageKey = `${sectionPath}.questions.${questionIndex}.image`;
    setValue(imageKey as keyof FormBuilderData, undefined as never);
  };
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        const { url, publicId } = await uploadImageToCloudinary(formData);
...
const ImageManager: React.FC<ImageManagerProps> = ({
  ...
  const handleSaveImage = () => {
    ...
    setValue(imagesKey as keyof FormBuilderData, currentImages as never);
...
interface SectionBuilderProps {
  ...
  setValue: UseFormSetValue<FormBuilderData>;
  getValues: UseFormGetValues<FormBuilderData>;
  onRemove: () => void;
  disabled?: boolean;
  isNested?: boolean;
  parentPath?: string;
...
const SectionBuilder: React.FC<SectionBuilderProps> = ({
  ...
  const addQuestion = () => {
    ...
    setValue(questionsKey as keyof FormBuilderData, updatedQuestions as never);
  };

  const handleTitleChange = (title: string) => {
    const titleKey = `${basePath}.title`;
    setValue(titleKey as keyof FormBuilderData, title as never);
  };

  const handleDescriptionChange = (description: string) => {
    const descriptionKey = `${basePath}.description`;
    setValue(descriptionKey as keyof FormBuilderData, description as never);
  };
  ...
  const handleAddSubsection = (isParent: boolean) => {
    ...
    setValue(subsectionsKey as keyof FormBuilderData, updatedSubsections as never);
  };
  ...
  const handleRemoveSubsection = (subIndex: number) => {
    ...
    const subsectionsKey = `${basePath}.subsections`;
    setValue(subsectionsKey as keyof FormBuilderData, currentSubsections as never);
  };
...
const TemplateCard: React.FC<TemplateCardProps> = ({
  ...
  const getTotalQuestions = (): number => {
    let total = 0;
    const countQuestions = (sections: Section[]) => {
...
const TemplateManagementApp: React.FC = () => {
  ...
  const handleSave = async (template: FormTemplate) => {
    ...
    if (currentView === "create") {
      const { _id, createdAt, updatedAt, ...createData } = template;
      result = await createTemplateHerraEquipo(createData);
    } else if (currentView === "edit" && selectedTemplate) {
      const { _id, createdAt, updatedAt, ...updateData } = template;
